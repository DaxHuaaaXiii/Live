...
//
    ...
    public void OtherApp(){
        if (REQUEST_CODE == 2){
//            if (listView.getVisibility() == View.VISIBLE){  //如果再加这个判断的话，startActivity(intentOtherApp)无法启动
//                getData();
//                mAdapter = new AppInfoAdapter(this,mList);
//                listView.setAdapter(mAdapter);
                listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        ...
                        Intent intentOtherApp;
                        //根據點擊到識別到的包名進行跳轉
                        intentOtherApp = getPackageManager().getLaunchIntentForPackage(appInfo.getPackageName());

                        if (intentOtherApp != null){
                            Log.d("hh", "onItemClick: ?????????");
                            startActivity(intentOtherApp);
                        }else {
                            Toast.makeText(MainActivity.this,"这个是系统插件，无法单独使用",
...
//        }
    ...
    public void getData() { //獲取所有第三方程序包名、圖標、app名字
        List<PackageInfo> packageInfos = getPackageManager().getInstalledPackages(0);
        if (packageInfos != null){
            for (int i = 0;i<packageInfos.size();i++){
                packageInfo = packageInfos.get(i);
                //過濾掉系統App
                // 但是這裏會出現一些零碎的組件，（但我不知道怎麽把這部分也過濾掉）
                // 這部分是依賴系統自帶app來運行的，所以儅過濾掉系統app之後，
                //這部分組件顯得沒有用，也沒有包名，所以等下在點擊調用的時候，
                //需要進行判斷，如果爲空不操作任何動作
                //不進行判斷的話會報空指針錯誤，導致閃退
                ...
                mList.add(appInfo);   //把所有信息給到mList這個集合
            }
        }
    }
    ...
    final BroadcastReceiver bluetoothDeviceHadFindReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                // 如果不是指定设备，直接返回
                if (!deviceName.equals("stalibizer_E001")) {
                    return;
                } // 必须使用stalibizer_E003这个名字过滤，不要使用Smart XR，不然会连不上设备
